...
void addToRequest(Group &gr, char *idMember)
{
	...
	// Kiểm tra xem có đủ không gian trong mảng không
	if (i < MAX_REQUEST)
	{
		// Sử dụng strncpy để tránh tràn bộ đệm
		strncpy(gr.request[i], idMember, sizeof(gr.request[i]) - 1);
		// Đảm bảo chuỗi kết thúc bằng null
		gr.request[i][sizeof(gr.request[i]) - 1] = '\0';
	}
	else
	{
		// Xử lý trường hợp không còn chỗ trống trong mảng request
		cout << "Không thể thêm vào request, mảng đã đầy." << endl;
	}
}
